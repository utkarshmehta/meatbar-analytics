...
const csvFilePath = path_1.default.resolve(__dirname, '../../data/data.csv');
const results = [];
...
function insertData() {
    const personStmt = database_1.default.prepare('INSERT OR IGNORE INTO people (name) VALUES (?)');
    // ---'INSERT OR IGNORE' ---
    const meatBarStmt = database_1.default.prepare('INSERT OR IGNORE INTO meat_bars (person_name, type, eaten_at) VALUES (?, ?, ?)');
    ...
    for (const row of results) {
        ...
        if (personName && meatBarType && eatenAt) {
            personStmt.run(personName, function () {
                if (this.changes > 0)
                    peopleInserted++;
            });
            meatBarStmt.run(personName, meatBarType, eatenAt, function () {
                if (this.changes > 0)
                    barsInserted++;
            });
        }
